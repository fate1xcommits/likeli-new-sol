...
import NegRiskConvert from "@/components/trade/NegRiskConvert";
        ...
        <div className="flex flex-col lg:flex-row min-h-[calc(100vh-64px)]">
            ...
            <div className="w-full lg:w-[400px] border-l border-[var(--border-subtle)] bg-[var(--bg-panel)] sticky top-0 h-screen overflow-y-auto">
                <div className="p-4 border-b border-[var(--border-subtle)]">
                    ...
                    {/* Arbitrage Opportunity Alert - Polymarket Style */}
                    {isMultiOutcome && multiMarket?.account.isOneWinner && (
                        <div className="mt-3">
                            {Math.abs(1 - probSum) > 0.01 && (
                                <div className="p-3 bg-purple-500/10 border border-purple-500/20 rounded-xl mb-3">
                                    ...
                                    <div className="text-sm text-purple-300">
                                        Sum of probabilities: <span className="font-bold">{(probSum * 100).toFixed(1)}%</span>
                                        <span className="text-purple-400/60"> (should be 100%)</span>
                                    </div>
                                    <div className="text-xs text-purple-400/60 mt-1">
                                        Convert NO positions below to profit from mispricing
                                    </div>
                                </div>
                            )}
                            <NegRiskConvert
                                marketPDA={multiMarket.publicKey}
                                answerCount={siblings.length}
                                answerLabels={siblings.map((s, i) => s.account.answerLabel || `Outcome ${i + 1}`)}
                                position={null}
