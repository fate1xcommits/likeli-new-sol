...
export function executeSandboxMultiArbitrageBuy(
    ...
    // Polymarket-style: Only update this answer's pool, no sibling rebalancing
    // Prices may temporarily not sum to 100%
    // Arbitrageurs use convert_positions() to profit from mispricing
    
    // Calculate fees
    const feeAmount = (amountUsd * market.feeBps) / 10000;
    const amountAfterFees = amountUsd - feeAmount;
    
    // Simple CPMM buy on this answer only
    const { answer: updatedAnswer, shares } = buyAnswerShares(
        market.answers[answerIndex],
        amountAfterFees
    );

    // Update only this answer's pool (NO sibling arbitrage)
    market.answers[answerIndex] = updatedAnswer;
    
    // Track position
    const posKeyYes = `${market.id}-${answerId}`;
    user.positions[posKeyYes] = (user.positions[posKeyYes] || 0) + shares;
    ...
    market.collectedFees = {
        creatorFee: market.collectedFees.creatorFee + feeAmount * 0.4,
        platformFee: market.collectedFees.platformFee + feeAmount * 0.5,
        liquidityFee: market.collectedFees.liquidityFee + feeAmount * 0.1
    };

    updateMarketPhase(market);

    log('PolymarketBuy', `Bought ${shares.toFixed(2)} shares on answer ${answerId}. Price: ${(updatedAnswer.prob * 100).toFixed(1)}%`, {
        note: 'Sum may not equal 100% - use convert_positions for arbitrage'
    });
